...
export const NOT   = notGate(gpt("notIn",    "notCenter")[0], gpt("notIn",    "notCenter")[1]);
export const XOR   = xorGate(gpt("xorLeft",  "center"  )[0], gpt("xorLeft",  "center"  )[1]);
// gap=100 → top gate at y=210, bot gate at y=310 — 60px between bodies for 45° cross wires
export const LATCH = srLatch(gpt("latchLeft", "center"  )[0], gpt("latchLeft", "center"  )[1], 100);
...
const PRE_HI: Point[] = [
  CONV, SPLIT1, gpt("split1", "xorTop"), gpt("xorLeft", "xorTop"),
  gpt("xorOut", "center"),
  gpt("circLeft", "center"),
];
...
const latchPathQ: Point[] = [
  [SPLIT2[0], TOP_PIN_Y], [LATCH_X, TOP_PIN_Y], [LATCH_X, Q_OUT[1]], Q_OUT,
];
const latchPathQBar: Point[] = [
  [SPLIT2[0], BOT_PIN_Y], [LATCH_X, BOT_PIN_Y], [LATCH_X, QBAR_OUT[1]], QBAR_OUT,
];

// Q/Q̄ output → right rail → top/bot arc → CONV
const SUF_Q:    Point[] = [gpt("feedRight", "latchTop"), gpt("feedRight", "feedTop"), gpt("feedLeft", "feedTop"), CONV];
const SUF_QBAR: Point[] = [gpt("feedRight", "latchBot"), gpt("feedRight", "feedBot"), gpt("feedLeft", "feedBot"), CONV];
